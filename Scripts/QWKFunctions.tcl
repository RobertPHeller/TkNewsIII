...
namespace eval QWK {
    ...
    snit::widgetadaptor QWKList {
        typevariable columnheadings -array {
            columnheadings(name,stretch) yes
            columnheadings(name,text) Name
            columnheadings(name,anchor) w
            columnheadings(size,stretch) no
            columnheadings(size,text) Size
            columnheadings(size,anchor) e
            columnheadings(size,width) 100
            columnheadings(timestamp,stretch) no
            columnheadings(timestamp,text) {Time Stamp}
            columnheadings(timestamp,anchor) w
            columnheadings(timestamp,width) 300
            columnheadings(fullname,stretch) yes
            columnheadings(fullname,text) {File Name}
            columnheadings(fullname,anchor) w
        }
        typevariable columns {name size timestamp fullname}
        typevariable QWKToSpoolProg [auto_execok [file join $::execbindir QWKToSpool]]
        ...
        typevariable SpoolToReplyProg [auto_execok [file join $::execbindir SpoolToReplyProg]]
        ...
        typevariable QWKInSpool [file normalize [option get . qwkInSpool QWKInSpool]]
        typemethod   QWKFile {spool} {
            return [file join $QWKInSpool "${spool}.qwk"]
        }
        typevariable QWKOutSpool [file normalize [option get . qwkOutSpool QWKOutSpool]]
        typemethod   QWKReply {spool} {
            return [file join $QWKOutSpool "${spool}.rep"]
        }
        typevariable QWKWorkDir [file normalize [option get . qwkWorkDir QWKWorkDir]]
        ...
        typevariable QWKArchiver "[option get . qwkArchiver QWKArchiver]"
        typemethod   Archiver {archive msgFile} {
            return "$QWKArchiver $archive $msgFile"
        }
        typevariable QWKUnarchiver "[option get . qwkUnarchiver QWKUnarchiver]"
